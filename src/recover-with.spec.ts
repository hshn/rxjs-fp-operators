...
describe('recoverWith()', () => {
  it('should not be recovered when its source is not failure', () => {
    let result, error;
    of(1).pipe(recoverWith(e => of(right(999)))).subscribe(_ => result = _, _ => error = _);
  ...
  it('should not be recovered with failed recovery', () => {
    let result, error;
    throwError(new Error('error')).pipe(recoverWith(e => throwError(new Error('recovery')))).subscribe(_ => result = _, _ => error = _);
